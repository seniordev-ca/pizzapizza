...
import { UserLogin, ResetPasswordSendEmail, SetNewPassword } from '../../actions/sign-up-actions';
...
export class SignInContainerComponent implements OnDestroy, OnInit {
	...
	constructor(
		...
		this.deeplinkCouponSubscription = this.store.pipe(select(fromCommon.getDeepLinkCoupon)).subscribe(data => {
		...
		this.userSummarySubscriptionRef = this.userSummary$.subscribe(user => {
			...
			if (user && !user.contactNumber.phoneNumber) {
	...
	onSocialLoginClick(platform: SocialPlatformEnum) {
		...
		this.socialLogicService.authState.subscribe((user) => {
			...
			if ((user.provider === platform) && (platform === SocialPlatformEnum.FACEBOOK)) {
				this.store.dispatch(new LoginWithFb(user.authToken, false));
			}

			if ((user.provider === platform) && (user.provider === SocialPlatformEnum.GOOGLE)) {
				this.store.dispatch(new LoginWithGoogle(user.authToken, false));
			}
		});
	...
	handleSignInDetailsEventEmitter(event: SignInEmitterInterface) {
		...
		if (event.action === SignInActionsEnum.onSignInProfile) {
			...
			this.store.dispatch(new UserLogin(loginRequest, false));
		}
